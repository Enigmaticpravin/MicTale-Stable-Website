...
export default function PaymentFailure() {
    ...
    const fetchShowDetails = async () => {
      ...
      try {
        ...
        const matchingShow = shows.find(show =>
          show.participants?.some(participant =>
            participant.fullName === paymentData.firstname &&
            participant.phoneNumber === paymentData.phone
          )
        );

        if (matchingShow) {
          const matchingParticipant = matchingShow.participants.find(participant =>
            participant.fullName === paymentData.firstname &&
            participant.phoneNumber === paymentData.phone
          );

          setShowDetails(matchingShow);
          setParticipantDetails(matchingParticipant);
        } else {
          console.log("No matching upcoming show found.");
        }
      } catch (error) {
  ...
  function formatDateTime(dateTime) {
    ...
    const formattedTime = `${hours.toString().padStart(2, '0')} ${ampm}`;
  ...
  const ticketData = {
    date: date,
    time: time,
    participantName: paymentData?.firstname,
    venue: showDetails?.location,
    actType: participantDetails?.performanceType,
    bookingId: participantDetails?.bookingId,
    eventName: showDetails?.name
  };
