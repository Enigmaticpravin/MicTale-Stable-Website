  ...
  return (
    <section className='relative w-full border-b border-zinc-900 bg-[#0b0c10]'>
      ...
      <div className='relative z-10 max-w-[1400px] mx-auto px-6 pt-16 pb-24 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center'>
        <div className='lg:col-span-7 flex flex-col items-start justify-between h-full min-h-[350px]'>
          <div className='grid grid-cols-5 hidden gap-2 sm:gap-4 max-w-2xl w-full'>
            {[
          ...
          <div>
{/* Added mx-auto for mobile, resets to md:mx-0 for desktop */}
<div 
...
<div className='lg:col-span-5 flex justify-end font-sans antialiased'>
  <div className='w-full max-w-[440px] bg-gradient-to-b from-[#0f172a] to-[#020617] border border-white/10 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden group/card'>
    ...
    <form className='space-y-6 z-10 relative' onSubmit={e => e.preventDefault()}>
      ...
      <div className='space-y-1.5'>
        ...
        <div className='relative rounded-xl flex items-center border border-white/10 hover:border-white/30 bg-white/5 backdrop-blur-md focus-within:border-[#F9E498]/40 transition-all duration-300'>
          <input
            ...
            className='w-full bg-transparent px-5 py-4 text-xs text-white placeholder-gray-500 focus:outline-none cursor-pointer tracking-wide'
          />
          <svg className='w-4 h-4 text-white/40 absolute right-5 pointer-events-none' fill='none' viewBox='0 0 24 24' stroke='currentColor' strokeWidth={1.5}>
      ...
      <button className='w-full py-4 px-4 rounded-full text-xs font-bold uppercase tracking-[0.15em] cursor-pointer active:scale-[0.98] bg-gradient-to-r from-[#B8860B] via-[#F9E498] to-[#D4AF37] text-black shadow-[0_0_20px_rgba(212,175,55,0.2)] hover:shadow-[0_0_30px_rgba(212,175,55,0.4)] transition-all duration-300'>
        Request Booking
      </button>
      ...
      <button className='w-full border border-white/10 hover:border-white/20 bg-white/5 hover:bg-white/10 text-white font-bold py-4 px-4 rounded-full text-xs tracking-[0.15em] uppercase transition-all duration-300 cursor-pointer backdrop-blur-md'>
